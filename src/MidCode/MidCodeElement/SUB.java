...
public class SUB extends MidCode
        implements ABSTRACT_DEF, ABSTRACT_USE {
    private String opt1;
    private String opt2;
    private String dst;

    public SUB(String opt1, String opt2, String dst) {
        super();
        this.opt1 = opt1;
        this.opt2 = opt2;
        this.dst = dst;
    ...
    public String createMips() {
        if (!Utility.isVar(opt1) && !Utility.isVar(opt2)) {
            ...
            retStr += String.format("li %s, %s\r\n",
                    Register.t2,
                    opt1);
            retStr += String.format("subu %s, %s, %s\r\n",Register.t1,Register.t2,Register.t0);
            retStr += String.format("sw %s, %s(%s)\r\n",
                    Register.t1,
                    Utility.getOffset(dst,belong),
                    Utility.getPointerReg(dst,belong));
            ...
            retStr += String.format("subiu %s, %s, %s\r\n",Register.t1,Register.t0,opt2);
            retStr += String.format("sw %s, %s(%s)\r\n",
                    Register.t1,
                    Utility.getOffset(dst,belong),
                    Utility.getPointerReg(dst,belong));
    ...
    public void createMipsOpt() {
        if (isDelete) {
            ...
            if (!Utility.isVar(opt1) && !Utility.isVar(opt2)) {
                ...
                MipsFactory.mipsCode += String.format("subu %s, %s, %s\r\n",rDst,r1,r2);
            }
            else if (Utility.isVar(opt1) && !Utility.isVar(opt2)) {
                Register r1 = Flow.applyReadReg(opt1,this);
                Register rDst = Flow.applyWriteReg(dst,this);
                MipsFactory.mipsCode += String.format("subiu %s, %s, %s\r\n",rDst,r1,opt2);
            }
            else {
                Register r1 = Flow.applyReadReg(opt1,this);
                Register r2 = Flow.applyReadReg(opt2,this);
